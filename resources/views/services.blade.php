<x-app-layout>
    <div class="bg-slate-950 min-h-screen py-24 px-6 relative overflow-hidden">
        {{-- Lueurs décoratives pour l'effet "Électrique" --}}
        ...
        <div class="max-w-6xl mx-auto relative z-10">
            ...
            <div class="mt-24 text-center">
                @auth
                {{-- Si l'utilisateur est connecté, on l'envoie vers le formulaire de création d'annonce --}}
                <a href="{{ route('properties.create') }}" class="inline-flex items-center gap-4 bg-white text-slate-950 font-black px-10 py-5 rounded-2xl uppercase tracking-widest text-xs hover:bg-blue-600 hover:text-white transition-all shadow-2xl shadow-white/5">
                    Lancer votre annonce avec nous
                    <span class="material-symbols-outlined text-sm">add_circle</span>
                </a>
                @else
                {{-- Si l'utilisateur est un invité (guest), on l'envoie vers la page d'inscription --}}
                <a href="{{ route('register') }}" class="inline-flex items-center gap-4 bg-white text-slate-950 font-black px-10 py-5 rounded-2xl uppercase tracking-widest text-xs hover:bg-blue-600 hover:text-white transition-all shadow-2xl shadow-white/5">
