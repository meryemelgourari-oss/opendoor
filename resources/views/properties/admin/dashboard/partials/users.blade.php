<div x-data="{ 
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-10">
            <form action="{{ route('admin.users.index') }}" method="GET" class="max-w-4xl">
                <div class="flex items-center gap-0 bg-white dark:bg-slate-900 rounded-2xl shadow-sm ring-1 ring-slate-200 dark:ring-slate-800 focus-within:ring-4 focus-within:ring-blue-600/10 transition-all overflow-hidden h-16">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Rechercher un nom ou un email..." class="flex-1 h-full border-none bg-transparent text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-0 text-sm">
                    ...
                    <div class="flex items-center gap-2 px-3 h-full bg-slate-50/50 dark:bg-slate-800/50 border-l border-slate-100 dark:border-slate-800">
                        @if(request('search'))
                        <a href="{{ route('admin.users.index') }}" class="flex items-center justify-center size-9 rounded-lg text-slate-400 hover:bg-rose-50 hover:text-rose-500 transition-all">
                        ...
                        <tr class="group hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
                            <td class="px-8 py-5">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <div class="font-bold text-slate-900 dark:text-white group-hover:text-blue-600 transition-colors">{{ $user->name }}</div>
                            ...
                            <td class="px-8 py-5">
                                <form action="{{ route('admin.users.toggle', $user) }}" method="POST">
                                    ...
                                    <button type="submit" class="flex items-center gap-3 group/toggle">
                                        <div class="w-12 h-6 rounded-full p-1 transition-colors duration-300 {{ $user->is_active ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-700' }}">
                                            <div class="size-4 bg-white rounded-full shadow-sm transition-transform duration-300 {{ $user->is_active ? 'translate-x-6' : '' }}"></div>
                                        </div>
                                        <span class="text-[10px] font-black uppercase tracking-widest {{ $user->is_active ? 'text-emerald-600' : 'text-slate-400' }}">
                            ...
                            <td class="px-8 py-5 export-ignore">
                                <div class="flex justify-end items-center gap-2">
                                    <button @click="editUser = { id: '{{ $user->id }}', name: '{{ $user->name }}', email: '{{ $user->email }}' }; showEditModal = true"
                                    ...
                                    <form action="{{ route('admin.users.destroy', $user) }}" method="POST" onsubmit="return confirm('Supprimer ce membre définitivement ?')">
                                        @csrf @method('DELETE')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
            <div class="p-8 rounded-[2rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center gap-6 shadow-sm">
                ...
                <div>
                    ...
                    <p class="text-3xl font-black text-slate-900 dark:text-white">{{ $stats['users_count'] ?? 0 }}</p>
                </div>
            </div>
            ...
            <div class="p-8 rounded-[2rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center gap-6 shadow-sm">
                ...
                <div>
                    ...
                    <p class="text-3xl font-black text-slate-900 dark:text-white">{{ $stats['active_users_count'] ?? 0 }}</p>
                </div>
            </div>
            ...
            <div class="p-8 rounded-[2rem] bg-slate-900 dark:bg-blue-600 flex items-center gap-6 shadow-2xl shadow-blue-600/20">
                ...
                <div>
                    ...
                    <p class="text-3xl font-black text-white leading-none mt-1">{{ $stats['inactive_users_count'] ?? 0 }}</p>
                </div>
            </div>
        </div>
