    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 mb-8 shadow-sm border border-slate-200/60 dark:border-slate-800 flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('admin.properties.create') }}" class="flex items-center gap-2 rounded-2xl h-14 px-8 bg-slate-900 dark:bg-blue-600 text-white font-black uppercase text-[11px] tracking-widest shadow-xl hover:scale-105 active:scale-95 transition-all">
        ...
        <div class="flex flex-wrap items-center gap-3 mb-8">
            <a href="{{ route('admin.properties.index') }}"
                class="px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all {{ !request('status') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'bg-white dark:bg-slate-900 text-slate-400 border border-slate-200 dark:border-slate-800' }}">
                Toutes ({{ $stats['properties_count'] }})
            </a>

            <a href="{{ route('admin.properties.index', ['status' => 'brouillon']) }}"
                class="px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all {{ request('status') == 'brouillon' ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/20' : 'bg-white dark:bg-slate-900 text-slate-400 border border-slate-200 dark:border-slate-800' }}">
                Brouillons ({{ $stats['draft_count'] }})
            </a>

            <a href="{{ route('admin.properties.index', ['status' => 'archivee']) }}"
                class="px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all {{ request('status') == 'archivee' ? 'bg-slate-600 text-white shadow-lg shadow-slate-600/20' : 'bg-white dark:bg-slate-900 text-slate-400 border border-slate-200 dark:border-slate-800' }}">
                Archivées ({{ $stats['archived_count'] }})
            </a>

            <a href="{{ route('admin.properties.index', ['status' => 'publiée']) }}"
                class="px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all {{ request('status') == 'publiée' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-white dark:bg-slate-900 text-slate-400 border border-slate-200 dark:border-slate-800' }}">
                Publiées ({{ $stats['active_users_count'] }})
            </a>
        </div>
                        ...
                        <tr class="group hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <div class="text-[10px] font-bold text-blue-500 flex items-center gap-1 views-field">
                                            ...
                                            <span>{{ number_format($property->views_count ?? 0) }} vues</span>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-5 text-center">
                                ...
                                <span class="status-label px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest 
                                    {{ $property->status == 'active' ? 'bg-emerald-100 text-emerald-600' : ($property->status == 'pending' ? 'bg-amber-100 text-amber-600' : 'bg-rose-100 text-rose-600') }}">
                            ...
                            <td class="px-8 py-5 text-right">
                                <div class="flex justify-end items-center gap-2">
                                    ...
                                    {{-- 3. APPROBATION (Check/Block) --}}
                                    <form action="{{ route('admin.properties.moderate', $property) }}" method="POST" class="inline">
                                        @csrf @method('PATCH')
                                    ...
                                    {{-- 4. ACTIONS RAPIDES (Brouillon / Archive / Publier) --}}
