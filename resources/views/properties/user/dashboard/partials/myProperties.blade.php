    ...
    <main class="flex-1 flex flex-col max-w-[1200px] mx-auto w-full px-4 lg:px-10 py-8" x-data="{ searchOpen: false }">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
            ...
            <a href="{{ route('properties.create') }}" class="flex items-center justify-center gap-2 rounded-2xl h-14 px-8 bg-blue-600 text-white text-sm font-black shadow-xl shadow-blue-200/50 hover:bg-blue-700 hover:scale-105 transition-all text-center">
        ...
        <div class="bg-white dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-slate-800/50 overflow-hidden shadow-sm">
            ...
            <div class="p-5 border-b border-slate-100 dark:border-slate-800/50 bg-slate-50/50 dark:bg-slate-900/50">
                <form action="{{ route('dashboard.my-properties') }}" method="GET" class="flex flex-col md:flex-row gap-5">
                    <div class="flex-1">
                    ...
                    <div class="flex gap-2.5 overflow-x-auto pb-2 md:pb-0">
                        {{-- Filtre : Toutes --}}
                        <a href="{{ route('dashboard.my-properties') }}"
                            class="flex h-12 items-center justify-center gap-2.5 rounded-xl border px-5 whitespace-nowrap {{ !request('status') ? 'bg-blue-50/50 text-blue-600 border-blue-200' : 'bg-white dark:bg-slate-900 text-slate-600 border-slate-200/60 dark:border-slate-700/50' }}">
                            <span class="text-sm font-bold">Toutes</span>
                            <span class="px-2 py-0.5 rounded-full text-[10px] font-black {{ !request('status') ? 'bg-blue-600 text-white' : 'bg-slate-200/70 text-slate-600' }}">{{ $stats['total'] }}</span>
                        </a>

                        {{-- Filtre : Publiées --}}
                        <a href="{{ route('dashboard.my-properties', ['status' => 'publiee']) }}"
                            class="flex h-12 items-center justify-center gap-2.5 rounded-xl border px-5 whitespace-nowrap {{ request('status') == 'publiee' ? 'bg-green-50/70 text-green-700 border-green-200' : 'bg-white dark:bg-slate-900 text-slate-600 border-slate-200/60 dark:border-slate-700/50' }}">
                            <span class="text-sm font-bold">Publiées</span>
                            <span class="px-2 py-0.5 rounded-full text-[10px] font-black {{ request('status') == 'publiee' ? 'bg-green-600 text-white' : 'bg-slate-200/70 text-slate-600' }}">{{ $stats['publiees'] }}</span>
                        </a>

                        {{-- Filtre : Brouillons --}}
                        <a href="{{ route('dashboard.my-properties', ['status' => 'brouillon']) }}"
                            class="flex h-12 items-center justify-center gap-2.5 rounded-xl border px-5 whitespace-nowrap {{ request('status') == 'brouillon' ? 'bg-orange-50/70 text-orange-700 border-orange-200' : 'bg-white dark:bg-slate-900 text-slate-600 border-slate-200/60 dark:border-slate-700/50' }}">
                            <span class="text-sm font-bold">Brouillons</span>
                            <span class="px-2 py-0.5 rounded-full text-[10px] font-black {{ request('status') == 'brouillon' ? 'bg-orange-600 text-white' : 'bg-slate-200/70 text-slate-600' }}">{{ $stats['brouillons'] }}</span>
                        </a>

                        {{-- Filtre : Archivées --}}
                        <a href="{{ route('dashboard.my-properties', ['status' => 'archivee']) }}"
                            class="flex h-12 items-center justify-center gap-2.5 rounded-xl border px-5 whitespace-nowrap {{ request('status') == 'archivee' ? 'bg-slate-100 text-slate-700 border-slate-200/60 dark:border-slate-700/50' : 'bg-white dark:bg-slate-900 text-slate-600 border-slate-200/60 dark:border-slate-700/50' }}">
                            <span class="text-sm font-bold">Archivées</span>
                            <span class="px-2 py-0.5 rounded-full text-[10px] font-black {{ request('status') == 'archivee' ? 'bg-slate-600 text-white' : 'bg-slate-200/70 text-slate-600' }}">{{ $stats['archivees'] }}</span>
                        </a>
                    </div>
                </form>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-widest border-b border-slate-100 dark:border-slate-800/50">
                            ...
                            <th class="px-6 py-5">Vues</th> {{-- COLONNE VUES --}}
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50 bg-white dark:bg-slate-950">
                        ...
                        <tr class="property-row group">
                            <td class="px-7 py-6">
                                <div class="flex items-center gap-5">
                                    ...
                                    <div class="flex flex-col gap-1 min-w-0">
                                        <a href="{{ route('properties.show', $property->id) }}" class="text-base font-black text-slate-950 dark:text-white group-hover:text-blue-600 transition-colors leading-tight truncate max-w-[280px]">
                                            {{ $property->title }}
                                        </a>
                                        <div class="flex items-center gap-2 text-slate-400">
                                            <span class="text-[11px] font-medium uppercase tracking-tight">{{ $property->city ?? 'Maroc' }}</span>
                                            <span class="size-1 rounded-full bg-slate-200"></span>
                                            <span class="text-[11px] font-medium uppercase tracking-tight capitalize text-blue-600">{{ $property->type_bien ?? 'Bien' }}</span>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-6">
                                ...
                                @endphp
                                <span class="status-badge inline-flex items-center px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border {{ $currentStyle }}">
                                    <span class="size-1.5 rounded-full bg-current mr-2 {{ $isPending ? 'animate-pulse' : '' }}"></span>
                                    @if($property->status === 'publiee')
                                    {{ $isPending ? 'En attente d\'approbation' : 'En ligne' }}
                                    @else
                                    {{ $property->status }}
                                    @endif
                                </span>
                            </td>
                            {{-- NOMBRE DE VUES POUR CHAQUE ANNONCE --}}
                            <td class="px-6 py-6">
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <span class="text-sm font-black text-slate-900 dark:text-slate-100">
                                        {{ number_format($property->views_count ?? 0, 0, ',', ' ') }}
                                    </span>
                                </div>
                            </td>
                            ...
                            <td class="px-7 py-6 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <a href="{{ route('properties.edit', $property->id) }}" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all" title="Modifier">
                                        <span class="material-symbols-outlined text-[20px]">edit</span>
                                    </a>
                                    <form action="{{ route('properties.destroy', $property->id) }}" method="POST" onsubmit="return confirm('Voulez-vous supprimer cette annonce ?');">
                                        @csrf @method('DELETE')
            ...
            <div class="p-5 border-t border-slate-100 dark:border-slate-800/50 bg-slate-50/30">
                <span class="text-[11px] font-black text-slate-400 uppercase tracking-widest">Total : {{ $myProperties->count() }} annonces</span>
            </div>
        </div>

        {{-- Section Statistiques avec TILT 3D --}}
            ...
            <div class="stat-card-3d bg-white dark:bg-slate-950 p-7 rounded-2xl border border-slate-100 dark:border-slate-800/50 shadow-sm relative overflow-hidden group"
                ...
                <div class="inner-content relative z-10 space-y-5">
                    ...
                    <div>
                        ...
                        <p class="text-4xl font-black text-slate-950 dark:text-white leading-none">{{ number_format($totalVues, 0, ',', ' ') }}</p>
                    </div>
                    <p class="text-xs text-slate-400 font-medium">Impact global de vos publications</p>
                </div>
            </div>

            {{-- Autres stats Card (Brouillons) --}}
            <div class="bg-white dark:bg-slate-950 p-7 rounded-2xl border border-slate-100 dark:border-slate-800/50 shadow-sm relative overflow-hidden group">
                ...
                <div class="inner-content relative z-10 space-y-5">
                    ...
                    <div>
                        ...
                        <p class="text-4xl font-black text-slate-950 dark:text-white leading-none">{{ $stats['brouillons'] }}</p>
                    </div>
                    <p class="text-xs text-slate-400 font-medium">Annonces prêtes à être publiées</p>
                </div>
            </div>

            {{-- Autres stats Card (Publiées) --}}
            <div class="bg-white dark:bg-slate-950 p-7 rounded-2xl border border-slate-100 dark:border-slate-800/50 shadow-sm relative overflow-hidden group">
                ...
                <div class="inner-content relative z-10 space-y-5">
                    ...
                    <div>
                        ...
                        <p class="text-4xl font-black text-slate-950 dark:text-white leading-none">
                            {{ $myProperties->where('status', 'publiee')->where('is_approved', true)->count() }}
                        </p>
                    </div>
