<x-app-layout>
    ...
    init() {
        ...
        {{-- MODIFIÉ : h-[60vh] sur mobile pour donner de l'espace au texte --}}
        class="perspective-container relative h-[60vh] md:h-[50vh] w-full bg-slate-950 overflow-hidden shadow-2xl">

        {{-- FRAMES 1 à 4 (Contenu visuel) --}}
        {{-- On utilise une boucle pour simplifier le code répétitif --}}
        @foreach([
        ...
        <div x-show="frame === {{ $index }}" x-cloak
            ...
            <img src="{{ asset('images/' . $data['img']) }}" class="absolute inset-0 w-full h-full object-cover opacity-40 md:opacity-50">
        ...
        {{-- FRAME 5: RECHERCHE (Optimisation Mobile) --}}
        <div x-show="frame === 5" x-cloak
            ...
            <div class="relative z-10 text-center w-full max-w-xl">
                ...
                {{-- Formulaire : Rendu propre sur Mobile --}}
                <form action="{{ route('properties.index') }}" method="GET"
                    ...
                    <input type="text" name="city" placeholder="Ville, quartier..."
                        @focus="isPaused = true"
                        {{-- MODIFIÉ : text-sm sur mobile pour éviter le zoom iOS --}}
        ...
        {{-- Indicateurs de frames (Plus petits sur mobile) --}}
    ...
    {{-- 2. CATÉGORIES BLOCS --}}
    {{-- MODIFIÉ : px-4 sur mobile, px-6 sur tablette, px-16 sur desktop + marge négative ajustée --}}
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-16 mt-8 md:mt-10 relative z-20 perspective-1000">
        {{-- MODIFIÉ : gap-4 sur mobile pour gagner de la place, gap-6 sur desktop --}}
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            @foreach([
            ['icon' => 'home', 'label' => 'Maison'],
            ['icon' => 'apartment', 'label' => 'Apparts'], {{-- Texte raccourci pour mobile --}}
            ...
            <div class="electric-card-container group"
                ...
                {{-- MODIFIÉ : p-4 sur mobile (au lieu de 6), arrondi légèrement réduit pour les petits écrans --}}
                <div class="electric-card relative bg-slate-900 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] shadow-xl flex items-center gap-3 md:gap-4 cursor-pointer transition-transform duration-150 ease-out will-change-transform">
                    ...
                    <div class="relative z-10 flex items-center gap-3 md:gap-4 w-full">
                        {{-- Icône style néon : MODIFIÉ taille w-10/h-10 sur mobile --}}
                        <div class="w-10 h-10 md:w-12 md:h-12 flex-shrink-0 bg-blue-950/50 border border-blue-500/30 text-blue-400 rounded-xl md:rounded-2xl flex items-center justify-center shadow-[0_0_15px_rgba(37,99,235,0.3)] group-hover:border-blue-400 group-hover:text-blue-300 transition-all duration-300 relative overflow-hidden">
                            ...
                            <span class="material-symbols-outlined text-xl md:text-2xl relative z-10">{{ $item['icon'] }}</span>
                        </div>

                        {{-- Texte style néon --}}
                        <div class="flex flex-col flex-1 overflow-hidden">
                            {{-- MODIFIÉ : text-[10px] sur mobile pour éviter que le texte ne saute à la ligne --}}
    ...
    {{-- SECTION PROPRIÉTÉS COMPACTE --}}
    <section class="max-w-5xl mx-auto px-6 py-12 space-y-20">

        {{-- 1. MAISONS : Style Cinématique réduit --}}
        @php $cat = ['id' => 'maison', 'title' => 'Dernières Maisons']; @endphp
        <div class="category-slide-group">
            <div class="flex justify-between items-end mb-6">
                <h2 class="text-2xl font-black text-slate-900 uppercase tracking-tighter">{{ $cat['title'] }}</h2>
                <a href="{{ route('properties.index', ['type_bien' => $cat['id']]) }}" class="text-[9px] font-bold text-blue-600 underline underline-offset-4 uppercase">Tout explorer</a>
            </div>
            <div class="flex gap-4 overflow-x-auto pb-6 snap-x no-scrollbar">
                @forelse($latestProperties->where('type_bien', $cat['id']) as $property)
                <div class="min-w-[320px] snap-start group relative h-[220px] rounded-[2rem] overflow-hidden shadow-xl">
                    <img src="{{ $property->ressources->first() ? asset('storage/' . $property->ressources->first()->resourceable->path) : asset('images/default.jpeg') }}" class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
                    <div class="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent"></div>
                    <div class="absolute bottom-0 p-6 w-full flex justify-between items-end">
                        <div>
                            <p class="text-blue-400 font-bold text-[10px] uppercase mb-0.5">{{ $property->city }}</p>
                            <h3 class="text-white text-base font-bold">{{ $property->title }}</h3>
                        </div>
                        <span class="bg-white text-slate-950 px-3 py-1.5 rounded-full font-black text-xs">{{ number_format($property->price, 0, ',', ' ') }} DH</span>
                    </div>
                </div>
                @empty
        ...
        {{-- 2. APPARTEMENTS : Grille Minimaliste resserrée --}}
        @php $cat = ['id' => 'appartement', 'title' => 'Appartements Récents']; @endphp
        <div class="category-slide-group">
            <div class="text-center mb-8">
                <span class="text-indigo-500 font-bold text-[9px] uppercase tracking-[0.4em]">SÉLECTION URBAN</span>
                <h2 class="text-2xl font-luxury italic font-extrabold text-slate-900">{{ $cat['title'] }}</h2>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @foreach($latestProperties->where('type_bien', $cat['id'])->take(3) as $property)
                <div class="group border-b border-slate-200 pb-4">
                    <div class="aspect-[4/5] rounded-xl overflow-hidden mb-3 bg-slate-100">
                        <img src="{{ $property->ressources->first() ? asset('storage/' . $property->ressources->first()->resourceable->path) : asset('images/default.jpeg') }}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500">
                    </div>
                    <div class="flex justify-between items-start">
                        <h3 class="font-bold text-slate-800 text-sm">{{ $property->title }}</h3>
                        <span class="text-indigo-600 font-black text-sm">{{ number_format($property->price, 0, ',', ' ') }} DH</span>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        {{-- 3. TERRAINS : Format Badge compact --}}
        @php $cat = ['id' => 'terrain', 'title' => 'Terrains & Lots']; @endphp
        <div class="bg-amber-50 rounded-[2rem] p-8 border border-amber-100">
            <div class="flex items-center gap-3 mb-6">
                ...
                <h2 class="text-xl font-black text-amber-900 uppercase">{{ $cat['title'] }}</h2>
            </div>
            <div class="flex gap-4 overflow-x-auto no-scrollbar">
                @foreach($latestProperties->where('type_bien', $cat['id']) as $property)
                <div class="min-w-[200px] bg-white p-4 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
                    <div class="w-full h-24 rounded-lg mb-3 bg-slate-200 overflow-hidden">
                        <img src="{{ $property->ressources->first() ? asset('storage/' . $property->ressources->first()->resourceable->path) : asset('images/default.jpeg') }}" class="w-full h-full object-cover">
                    </div>
                    <div class="flex items-center gap-2 mb-2">
                        <span class="bg-amber-100 text-amber-700 text-[9px] font-bold px-2 py-0.5 rounded-md">{{ $property->surface }} m²</span>
                    </div>
                    <h3 class="font-bold text-slate-800 text-xs truncate mb-3">{{ $property->title }}</h3>
                    <a href="{{ route('properties.show', $property->id) }}" class="text-amber-600 font-bold text-[10px] flex items-center gap-1">Détails <span class="material-symbols-outlined text-xs">east</span></a>
                </div>
                @endforeach
            </div>
        </div>
        {{-- 4. LOCAUX COMMERCIAUX : Style Business Pro --}}
        @php $cat = ['id' => 'commercial', 'title' => 'Locaux Commerciaux']; @endphp
        <div class="category-slide-group">
            <div class="flex items-center justify-between mb-8">
                ...
                <a href="{{ route('properties.index', ['type_bien' => $cat['id']]) }}" class="group flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-blue-600 transition-colors">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @forelse($latestProperties->where('type_bien', $cat['id'])->take(4) as $property)
                <div class="flex bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 group">
                    <div class="w-1/3 relative overflow-hidden">
                        <img src="{{ $property->ressources->first() ? asset('storage/' . $property->ressources->first()->resourceable->path) : asset('images/default.jpeg') }}" 
                    ...
                    <div class="w-2/3 p-5 flex flex-col justify-between">
                        <div>
                            <div class="flex justify-between items-start mb-2">
                                <span class="text-blue-600 font-bold text-[10px] uppercase">{{ $property->city }}</span>
                                <span class="font-black text-slate-900 text-sm">{{ number_format($property->price, 0, ',', ' ') }} DH</span>
                            </div>
                            <h3 class="font-bold text-slate-800 text-base leading-tight mb-2 group-hover:text-blue-600 transition-colors">{{ $property->title }}</h3>
                            <p class="text-slate-500 text-xs line-clamp-2">{{ Str::limit($property->description, 70) }}</p>
                        </div>
                        <div class="flex items-center gap-4 pt-4 border-t border-slate-50">
                            <div class="flex items-center gap-1 text-slate-400">
                                <span class="material-symbols-outlined text-sm">straighten</span>
                                <span class="text-[10px] font-bold">{{ $property->surface }} m²</span>
                            </div>
                            <a href="{{ route('properties.show', $property->id) }}" class="ml-auto text-slate-900 font-bold text-[10px] uppercase tracking-wider">Découvrir</a>
                        </div>
                    </div>
                </div>
                @empty
    ...
    <section class="max-w-5xl mx-auto px-6 py-10">
        <div class="relative rounded-[2.5rem] overflow-hidden bg-slate-950 p-10 shadow-2xl">
            <div class="absolute top-0 right-0 w-1/3 h-full opacity-10 pointer-events-none">
                <img src="{{ asset('images/porte.png') }}" class="w-full h-full object-cover grayscale">
            </div>
            <div class="relative z-10 max-w-xl">
                ...
                <div class="flex gap-3">
                    @auth
                    <a href="{{ route('dashboard.index') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase transition-all flex items-center gap-2">
                        <span class="material-symbols-outlined text-sm">add_circle</span> Publier
                    </a>
                    @else
                    <a href="{{ route('register') }}" class="bg-white text-slate-950 px-6 py-3 rounded-xl text-[10px] font-black uppercase flex items-center gap-2">
        ...
        <div class="max-w-7xl mx-auto px-8 relative z-10">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-12 mb-8">
                ...
                <div>
                    ...
                    <ul class="space-y-2">
                        @foreach(['Services' => route('services'), 'À propos' => route('about'),'Mentions Legales'=> route('legal'), 'Contact' => route('contact')] as $l => $u)
                        <li><a href="{{ $u }}" class="text-[11px] font-bold text-slate-400 hover:text-white transition-colors">{{ $l }}</a></li>
                        @endforeach
                    </ul>
                </div>
                ...
                <div class="bg-blue-900/20 backdrop-blur-md p-6 rounded-[1.5rem] border border-white/5">
                    ...
                    <div class="flex items-baseline gap-2">
                        <span class="text-3xl font-black text-white tracking-tighter">{{ $newPropertiesCount }}</span>
                        <span class="text-blue-300 text-[8px] uppercase font-black">{{ Str::plural('Opportunité', $newPropertiesCount) }}</span>
                    </div>
                </div>
