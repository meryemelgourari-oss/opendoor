...
<main class="flex-1 overflow-y-auto p-2 lg:p-4 bg-slate-50 dark:bg-slate-950">
    ...
    <form action="<?php echo e(route('admin.dashboard')); ?>" method="GET" class="flex flex-wrap items-end gap-3 print:hidden bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200/50 mb-6">
        <div class="flex flex-col gap-1">
            <label class="text-[10px] font-black uppercase text-slate-400 ml-2">Du</label>
            <input type="date" name="date_from" value="<?php echo e(request('date_from')); ?>" class="bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-xs font-bold focus:ring-2 focus:ring-blue-500 transition-all">
        </div>
        <div class="flex flex-col gap-1">
            <label class="text-[10px] font-black uppercase text-slate-400 ml-2">Au</label>
            <input type="date" name="date_to" value="<?php echo e(request('date_to')); ?>" class="bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-xs font-bold focus:ring-2 focus:ring-blue-500 transition-all">
        </div>
        ...
        <a href="<?php echo e(route('admin.dashboard')); ?>" class="text-[10px] font-black uppercase text-rose-500 hover:underline mb-3 ml-2">Effacer</a>
        <?php endif; ?>
    </form>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
        ...
        ?>
        <?php $__currentLoopData = $mini_stats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ms): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="bg-white dark:bg-slate-900 p-4 rounded-[2rem] border border-slate-200/50 shadow-sm flex items-center gap-4 transition-transform hover:scale-[1.02]">
            <div class="w-10 h-10 rounded-2xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center <?php echo e($ms['color']); ?>">
                <span class="material-symbols-outlined"><?php echo e($ms['icon']); ?></span>
            </div>
            <div>
                <p class="text-[9px] font-black uppercase text-slate-400"><?php echo e($ms['label']); ?></p>
                <h3 class="text-lg font-black dark:text-white"><?php echo e(number_format($ms['val'])); ?></h3>
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-slate-200/60 shadow-2xl mb-12">
        <div class="mb-8">
            ...
            <p class="text-xs text-slate-400 font-medium italic">
                <?php echo e(request('date_from') && request('date_to') 
    ...
    <div class="w-full flex flex-col gap-8 mb-8">
                ...
                <div class="p-5 rounded-[2rem] bg-slate-50 dark:bg-slate-800/50 hover:bg-indigo-50 transition-all border border-transparent hover:border-indigo-100 flex flex-col justify-between">
                    <div>
                        <div class="flex justify-between items-start mb-2">
                            ...
                            <?php echo e($rec->status === 'resolu' ? 'bg-emerald-100 text-emerald-600' : ($rec->status === 'en_cours' ? 'bg-amber-100 text-amber-600' : 'bg-rose-100 text-rose-600 animate-pulse')); ?>">
                                <?php echo e($rec->status); ?>

                            </span>
                            <span class="text-[9px] font-bold text-slate-400"><?php echo e($rec->created_at->diffForHumans()); ?></span>
                        </div>

                        <h5 class="font-bold text-sm mb-1 dark:text-white"><?php echo e($rec->subject); ?></h5>
                        <p class="text-xs text-slate-500 line-clamp-2 mb-4"><?php echo e($rec->message); ?></p>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center gap-2">
                            <div class="w-6 h-6 rounded-lg bg-slate-300 flex items-center justify-center text-[10px] font-bold text-white uppercase">
                                <?php echo e(substr($rec->user->name ?? '?', 0, 1)); ?>

                            </div>
                            <span class="text-[10px] font-bold text-slate-600 dark:text-slate-400"><?php echo e($rec->user->name ?? 'Anonyme'); ?></span>
                        </div>

                        <form action="<?php echo e(route('admin.reclamations.update', $rec)); ?>" method="POST" class="w-full">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('PATCH'); ?>

                            <?php if($rec->status === 'ouvert'): ?>
                            ...
                            <?php elseif($rec->status === 'en_cours'): ?>
        ...
        <div id="testimonials-section" class="w-full bg-white dark:bg-slate-900 rounded-[3rem] p-8 border border-slate-200/60 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[600px] overflow-y-auto custom-scrollbar pr-2">
                ...
                <div class="p-5 rounded-[2rem] border <?php echo e($testimony->is_approved ? 'border-emerald-100 bg-emerald-50/20' : 'border-amber-100 bg-amber-50/20'); ?>">
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-10 h-10 rounded-xl bg-slate-900 flex items-center justify-center text-white font-black text-xs">
                            <?php echo e(substr($testimony->name, 0, 1)); ?>

                        </div>
                        <div>
                            <p class="text-xs font-black dark:text-white"><?php echo e($testimony->name); ?></p>
                            <p class="text-[9px] text-slate-400 uppercase font-bold"><?php echo e($testimony->created_at->translatedFormat('d M Y')); ?></p>
                        </div>
                    </div>
                    <p class="text-xs italic text-slate-600 dark:text-slate-400 mb-4 font-medium">"<?php echo e($testimony->content); ?>"</p>
                    <div class="flex gap-2">
                        <form action="<?php echo e(route('admin.testimonials.update', $testimony)); ?>" method="POST" class="flex-1">
                            <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                            <input type="hidden" name="status" value="<?php echo e($testimony->is_approved ? 'pending' : 'approved'); ?>">
                            <button class="w-full py-2.5 rounded-xl text-[9px] font-black uppercase tracking-tighter transition-all <?php echo e($testimony->is_approved ? 'bg-slate-200 text-slate-600' : 'bg-emerald-500 text-white shadow-lg'); ?>">
                                <?php echo e($testimony->is_approved ? 'Désactiver' : 'Approuver'); ?>

                            </button>
                        </form>
                        <form action="<?php echo e(route('admin.testimonials.destroy', $testimony)); ?>" method="POST" onsubmit="return confirm('Supprimer cet avis ?')">
    ...
    <style>
        ...
        @media print {
            ...
            header::after {
                content: "Période : <?php echo e(request('date_from', 'Début')); ?> - <?php echo e(request('date_to', 'Aujourd\'hui')); ?>";
                display: block;
                font-size: 12px;
                margin-top: 10px;
                font-weight: bold;
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: [
                        <?php echo e($stats['active_users_count'] ?? 0); ?>,
                        <?php echo e($stats['inactive_users_count'] ?? 0); ?>,
                        <?php echo e($stats['published_count'] ?? 0); ?>,
                        <?php echo e($stats['archived_count'] ?? 0); ?>,
                        <?php echo e($stats['draft_count'] ?? 0); ?>
...
<?php /**PATH C:\Users\HP\Desktop\opendoor v-final\opendoor\resources\views/properties/admin/dashboard/partials/overview.blade.php ENDPATH**/ ?>